...
use crate::{gdt::GDT, paging::IDENT_PAGING};
...
pub fn enter_compat_mode() {
    ...
    unsafe {
        asm!(
            ...
            p4_table = in(reg) &raw const IDENT_PAGING.try_lock().expect("Failed to lock IDENT_PAGING").level_4 as *mut u8,
        )
    }
...
pub fn enable_long_mode() {
    ...
    IDENT_PAGING
        .try_lock()
        .expect("Failed to lock IDENT_PAGING")
        .setup();
